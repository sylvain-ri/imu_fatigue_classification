...
def stats_to_pandas(compare_these_states=(LABELS["walk_rested"], LABELS["walk_very_tired"])):
    ...
    for f in tqdm(all_frames, total=len(all_frames)):
        ...
        if f['label'] not in compare_these_states:
...
def fft_features_to_pandas(compare_these_states=(LABELS["walk_rested"], LABELS["walk_very_tired"]), n=5):
...
# ####################         Random Forest         ####################
# #######################################################################
def rf_classification(trees=50):
...
def features_coefs(features_names, cl_coef, top=8):
    """Print the top n most important coef for the classification"""
    feat_importance = pd.DataFrame({'feat_x': features_names, 'importance': cl_coef})
    feat_importance.sort_values(['importance'], ascending=False, inplace=True)
    print(feat_importance.head(top))


def print_cross_predictions(expected_y, predicted_y):
    """Print the predictions in a cross table"""
    print(pd.crosstab(expected_y, predicted_y, rownames=['Actual Label'], colnames=['Predicted Label ->']))
